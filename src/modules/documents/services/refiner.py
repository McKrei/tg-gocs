...
async def refine_draft(old_draft: dict[str, Any], user_feedback: str) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "ВАЖНО: Если текущий черновик содержит сообщение об ошибке парсинга JSON или некорректные временные "
        "данные (например, имя файла в виде хэша/кода), попытайся извлечь корректные данные из исходного "
        "текста ошибки, упомянутого в поле summary текущего черновика, и применить к ним замечание пользователя.\n"
        "Также никогда не генерируй длинные последовательности повторяющихся символов (особенно нулей) "
        "и технические коды целиком. Если они длиннее 10 символов, сокращай их (например, '0... (20 нулей)').\n\n"
