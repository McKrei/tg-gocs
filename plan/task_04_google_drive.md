...
>
> Агент подготовил пошаговую инструкцию.
...
- [x] `src/drive/client.py` — авторизация через Service Account
- [x] `src/drive/uploader.py` — логика загрузки:
  1. Проверить существование папки по пути (напр. `Медицина/Жена`)
  2. Создать папки если не существуют
  3. Загрузить файл
  4. Вернуть `webViewLink`
- [x] Подключить Drive к инструменту `save_to_local_and_drive` (убрать заглушку)
- [x] `tests/test_drive.py` — интеграционный тест (с реальным Drive)
- [x] Инструкция по настройке Service Account в `docs/google_drive_setup.md`
